...
def _initial_conflicts_payload() -> tuple[Any, str, Any]:
    project = get_project()
    db = project.db
    if project.config.screening.workflow == "assisted":
        conflicts = db.list_assisted_conflicts(project.project_id)
    else:
        conflicts = db.list_screening_conflicts(project.project_id)
    if conflicts:
        cards = [_conflict_card(s, db) for s in conflicts]
        count_text = f"{len(conflicts)} unresolved conflict(s)"
    else:
        cards = [dbc.Alert("No unresolved conflicts. ✓", color="success")]
        count_text = "0 unresolved"
    resolved = db.list_reconciliations(project.project_id, stage="screening", limit=10)
    resolved_ui = _resolved_list(resolved) if resolved else html.P("(none yet)", className="small text-muted")
    return cards, count_text, resolved_ui


def layout() -> Any:
    cards, count_text, resolved_ui = _initial_conflicts_payload()
    return dbc.Row(
        [
            dbc.Col(
                [
                    html.H6("Resolve conflicts", className="fw-bold"),
                    html.P(
                        (
                            "Assisted mode: sources where the AI and the human reviewer disagreed."
                            if get_project().config.screening.workflow == "assisted"
                            else "Independent mode: sources where two human reviewers disagreed."
                        )
                        + " Pick the final decision; recorded with you as adjudicator.",
                        className="small text-muted",
                    ),
                    html.Hr(),
                    html.Div(count_text, id="conflicts-counts", className="small text-muted"),
                ],
                width=3,
            ),
            dbc.Col(
                [
                    html.Div(cards, id="conflicts-cards"),
                    html.Hr(className="mt-4"),
                    html.H6("Recently resolved", className="fw-bold"),
                    html.Div(resolved_ui, id="conflicts-resolved"),
                ],
                width=9,
            ),
        ]
...
def register_callbacks(app: Any) -> None:
    @app.callback(
        Output("conflicts-refresh", "data"),
        Input({"type": "conflict-decide", "source": ALL, "decision": ALL}, "n_clicks"),
        State("shared-reviewer", "value"),
        State({"type": "conflict-rationale", "source": ALL}, "value"),
        State({"type": "conflict-rationale", "source": ALL}, "id"),
    ...
    def _on_resolve(_clicks, reviewer, rationales, rationale_ids):
        ...
        if not isinstance(triggered, dict) or triggered.get("type") != "conflict-decide":
            return no_update
        ...
        db = get_project().db
        db.insert_screening_reconciliation(source_id, decision, rid, rationale)
        db.insert_screening_action(source_id, rid, action="reconcile", decision=decision)
        return {"ts": time.time()}

    @app.callback(
        Output({"type": "conflict-abstract-body", "source": ALL}, "is_open"),
        Input({"type": "conflict-abstract-btn", "source": ALL}, "n_clicks"),
        State({"type": "conflict-abstract-body", "source": ALL}, "is_open"),
        prevent_initial_call=True,
    )
    ...
    @app.callback(
        Output("conflicts-refresh", "data", allow_duplicate=True),
        Input({"type": "conflict-undo", "rec_id": ALL}, "n_clicks"),
        prevent_initial_call=True,
    )
    def _on_undo(_clicks):
        ...
        if row:
            db.insert_screening_action(
                row["source_id"], row["adjudicator"], action="reconcile_undo"
            )
        return {"ts": time.time()}

    @app.callback(
        Output("conflicts-cards", "children"),
        Output("conflicts-counts", "children"),
        Output("conflicts-resolved", "children"),
        Input("conflicts-refresh", "data"),
    ...
    def _render(_refresh, reviewer):
        ...
        return _initial_conflicts_payload()


def _conflict_card(src: Source, db: Any) -> Any:
    sid = src.id
    decisions = db.get_human_decisions(sid)

    vote_rows: list[Any] = []
    ai = db.get_latest_ai_decision(sid, stage="abstract")
    if ai is not None:
        vote_rows.append(ai_detail_block(ai))
    for d in decisions:
        color = _DECISION_COLORS.get(d["decision"], "secondary")
        reasoning = (d.get("reasoning") or "").strip()
        if reasoning == "(inline screening)":
            reasoning = ""
        vote_rows.append(
            html.Div(
                [
    ...
    final_btns = dbc.ButtonGroup(
        [
            dbc.Button(
                "Include",
                id={"type": "conflict-decide", "source": sid, "decision": "include"},
                color="success",
                size="sm",
            ),
            dbc.Button(
                "Exclude",
                id={"type": "conflict-decide", "source": sid, "decision": "exclude"},
                color="danger",
                size="sm",
            ),
        ]
    ...
    abstract_btn = html.Div(
        dbc.Button(
            "Abstract ▼",
            id={"type": "conflict-abstract-btn", "source": sid},
            size="sm",
            color="link",
            className="p-0",
        ),
    ...
    abstract_body = dbc.Collapse(
        html.P(
            src.abstract or "(no abstract)",
            className="mt-2 small",
            style={"whiteSpace": "pre-wrap"},
        ),
        id={"type": "conflict-abstract-body", "source": sid},
        is_open=False,
    )
    ...
    history_btn = html.Div(
        dbc.Button(
            "History",
            id={"type": "conflict-history-btn", "source": sid},
            size="sm",
            color="link",
            className="p-0",
        ),
                        ...
                        dbc.Row(
                            [
                                ...
                                dbc.Col(
                                    dbc.Button(
                                        "Undo",
                                        id={"type": "conflict-undo", "rec_id": r["id"]},
                                        color="link",
                                        size="sm",
                                    ),
                                    width=2,
