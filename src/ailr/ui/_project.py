"""The active project: loading it, switching it, and reading the files that belong to it.

One Project per process, cached in `_project` and named by the AILR_PROJECT environment variable
so background threads (see ai_runner) can reach it too. Anything that rewrites lit_review.yaml
must call reload_project() afterwards, or the running app keeps the stale config.
"""
