...
Linked PDFs are converted to markdown so the AI (and you) can read the full text. The default backend is `pymupdf` (a **backend selector** on the Workflow tab lets you switch to `marker`); references are stripped by default to keep the text focused on the study itself. Conversion runs several PDFs **in parallel** (a *Parallel workers* setting, default 4).

The Workflow tab gives you re-conversion controls when a PDF comes in badly:
