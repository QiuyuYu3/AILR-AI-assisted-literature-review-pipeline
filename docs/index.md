...
:::{note}
This handbook is task-oriented. If you are setting up for the first time, read [Getting started](getting-started.md) and [Core concepts](concepts.md), [set up your protocol](protocol.md) (criteria and variables), then work through the **Running a review** chapters in order. They follow the left sidebar of the app.
:::
...
| Stage | What happens |
|-------|--------------|
| **Set up your protocol** | define your inclusion/exclusion **criteria** and extraction **variables** once, on the Protocol page. They drive every stage that follows |
...
You define the review once: the criteria, the screening and extraction prompts, the fields to extract, and the workflow mode. Then you calibrate the AI until it tracks your judgement. The tool never writes that domain content for you; it gives it a pipeline to run in and an audit trail around it.
...
- **An API key** for your AI provider (Anthropic / OpenAI / Gemini) to run the real AI. Without one you can still use **Mock** mode to click through the whole app with no key and no cost.
...
:::{card} Set up your protocol
:link: protocol.md
Define your criteria and extraction variables, the shared definitions everything depends on.
:::
